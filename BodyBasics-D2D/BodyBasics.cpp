...
void CBodyBasics::Update()
{
    ...
    {
		MessageBox(m_hWnd, L" If you are reading this message, Kinect is  not recognized bois", L"Frame Reader func not working", MB_ICONEXCLAMATION);
        return;
    }
...
HRESULT CBodyBasics::InitializeDefaultSensor()
{
    ...
    if (FAILED(hr))
    {
		MessageBox(m_hWnd, L" If you are reading this message, Kinect is  not recognized bois", L"Unable to find Kinect", MB_ICONEXCLAMATION);
        return hr;
    }
    ...
    if (!m_pKinectSensor || FAILED(hr))
    {
		...
		MessageBox(m_hWnd, L" If you are reading this message, Kinect is  not recognized bois", L"Stuff is not working I guess", MB_ICONEXCLAMATION);
        return E_FAIL;
    }
    ...
    if (m_hWnd)
    {
        ...
        if (SUCCEEDED(hr) && m_pRenderTarget && m_pCoordinateMapper)
        {
			...
			if (flagS == true) 
			{
				...
				StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L" FPS = %0.2f   Time = %I64d   SDJv0 DEBUG--Skeleton found", fps, (nTime - m_nStartTime));
				flagS == false;
			}
			else 
			{

				StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L" FPS = %0.2f   Time = %I64d   SDJv0 DEBUG--Skeleton lost", fps, (nTime - m_nStartTime));
            ...
            if (D2DERR_RECREATE_TARGET == hr)
            {
                ...
                DiscardDirect2DResources();
				flagS = false;
				StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L" FPS = %0.2f  Time = %I64d   SmarDJv0 DEBUG--Skeleton lost", fps, (nTime - m_nStartTime));
            }
        }
...
void CBodyBasics::DrawHand(HandState handState, const D2D1_POINT_2F& handPosition)
{
    ...
    switch (handState)
    {
        ...
        case HandState_Lasso:
			...
			MessageBox(m_hWnd, L"About to get directed to a the website, stop making lasso or it will burry you under websites", L"GESTURE TEST", MB_ICONEXCLAMATION);
			MessageBox(m_hWnd, L"Like for real, this thing will crash your computer", L"GESTURE TEST", MB_ICONEXCLAMATION);
			ShellExecute(m_hWnd, L"open", L"http://smartdj.localtunnel.me", NULL, NULL, SW_SHOWNORMAL);
            break;
    }
}
