...
To-do: Get the POST http request-ask Chris for what request exactly

Set up the browser call function so it executes once, and it executes only every 3-5 secs
...
namespace Microsoft.Samples.Kinect.DiscreteGestureBasics
{
    ...
    public sealed class GestureResultView : INotifyPropertyChanged
    {
        ...
        /// <summary> True, if the web browser is already open </summary>
        ...
        public void GestureResponse(bool isBodyTrackingIdValid, bool isGestureDetected, float detectionConfidence, bool whichGesture)
        {
            if(isBodyTrackingIdValid == true && isGestureDetected == true && detectionConfidence > 0.3 && isCalled == false)
            {
                //Previous song
                if (whichGesture == true)
                {
                    System.Diagnostics.Process.Start("http://www.csun.edu"); ///////////////////Web player https goes in here
                    isCalled = true;
